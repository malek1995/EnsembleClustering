...
def create_ensemble_clustering(k_list, itr_num, ensemble_input, algo_name):
    ...
    for k in k_list:
        for i in range(itr_num):
            label = CE.cluster_ensembles(ensemble_input, nclass=k, solver=algo_name.lower())
...
def add_validation_results(data, target, labels, algo_name):
    ...
    internal_mean = calculate_mean_of_validation(internal)
    internal_variance = calculate_variance_of_validation(internal, internal_mean)

    external = calculate_external_validation_for_all_labels(target, labels)
    external_mean = calculate_mean_of_validation(external)
    external_variance = calculate_variance_of_validation(external, external_mean)

    add_results(algo_name, internal_mean, external_mean, internal_variance, external_variance)
...
def perform_ensemble_clustering(base_models, k_list, itr_num, data, target):
    ensemble_methods = ['NMF', 'CSPA', 'MCLA', 'HBGF']
    for method in ensemble_methods:
        ensemble_input = modify_models_to_valid_ensemble_input(base_models)
        ensemble_labels = create_ensemble_clustering(k_list, itr_num, ensemble_input, method)
